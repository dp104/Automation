...
test('Global search application flow', async({page}) => {
    ...
    await page.getByText('Apply Now', { exact: true }).nth(8).click(); //course card from the results
    await page.locator('tr:has-text("GUIDS131") >> text=Select').click(); //126,190,128
    // await page.locator('tr', {
    // has: page.locator('td', { hasText: /^GUIDS131$/ })
    // }).locator('button:has-text("Select")').click();
