...
test('Dashboard test', async({page}) => {
    ...
    await menu.click();
    await page.getByText('Add Student(Old)',{exact:true}).click();
    await page.waitForTimeout(3000);
    // await expect(page).toHaveURL(/add-student/);
    console.log("Add student Information page is opened");
    //Form open 
    await page.locator('input[placeholder="First name"]').fill('prasad'); //Studnet First Name
    await page.locator('input[placeholder="Last name"]').fill('devarakonda');  //Studnet Last Name
    await page.locator('input[type="email"]').fill('prasaddevarakonda@mailinator.com'); //Student Mail id 
    await page.locator('css-1ljjm92-placeholder').click();
    await page.getByText('Male', { exact: true }).click(); // Gender selecting --> Female or Male
    // await page.screenshot({ path: 'screenshot.png', fullPage: true });
    await page.locator('input[type="date"]').fill('2000-01-15');  //Student DOB 
    await page.locator('body').click();
