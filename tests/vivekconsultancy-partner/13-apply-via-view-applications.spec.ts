...
// Partner Portal — Apply via View Applications (#/Get-Applications).
// Same .gad-* View Applications component and "Create New Application" popup
// the student portal uses on its own applications page (tests/student-flows/36)
// — mechanically nothing differs here either. The one real difference: the
// student portal expands "the" student's own row, but the partner's view is
// an AGGREGATE list across the whole roster (confirmed in test 05), so this
// script simply expands the FIRST student row it finds and creates the new
// application on that student's behalf. Which student ends up with the new
// application is incidental for this coverage test.

// Flow 4 — View Applications:
// Get-Applications page → expand the first student row (chevron) →
...
test('Partner Portal — Apply via View Applications (Create New Application)', async ({ page }) => {
    ...
    // ── Open View Applications and expand the first student row ───────────────
    ...
    // the click handler runs — the submission path itself silently aborts. This
    // was first documented from the student portal (tests/student-flows/36); the
    // partner portal drives the identical .gad-popup component and handler, so
    // the defect is expected to reproduce here too, independent of role.
    ...
    console.log('\n⚠ APP DEFECT (report to GuideUni team): "Create New Application" popup (shared by student and partner portals) —');
