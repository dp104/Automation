...
// Partner Portal — Apply via Dashboard Top Search.
// Same header "Search Course" box and same Search-Course/Apply-Now components
// the student portal uses (tests/student-flows/33) — confirmed present on the
// partner dashboard too. Mechanically nothing differs: the only real
// difference is WHO the application gets created for — the Student Profile
// Journey wizard that "Apply Now" opens is the partner acting on behalf of
// whichever student row is in context, not the partner's own profile.
...
test('Partner Portal — Apply via Dashboard Top Search', async ({ page }) => {
