...
// Partner Portal — Create Application + Upload Documents (existing student).
// Same flow as tests/student-flows/32-create-application-with-documents.spec.ts
// and the same "fetch an existing student" mechanic as partner test
// 04-create-application-existing-student.spec.ts, continued through to the
// wizard's "Documents" tab via the shared uploadAllRequiredDocuments helper.
// Nothing mechanically differs for a partner here — Create Application and
// Documents are the same sp-wizard component regardless of role — the only
// difference is WHO it's for: the partner is acting on behalf of whichever
// student row is in context, not the student doing it themselves.
...
test('Partner Portal — Create Application + Upload Documents for Existing Student', async ({ page }) => {
    ...
    // ── Open Student Profile Journey and load the configured student ─────────
    ...
    // ════════════════════════════════════════════════════════════════════════
    // PHASE 1 — CREATE APPLICATION (same approach as partner test 04 /
    // student-flows 29-create-application)
    // ════════════════════════════════════════════════════════════════════════
    ...
    console.log(`\n✅ Application created (${baseline.count} → ${after.count})${newAppIds.length ? ` — ${newAppIds.join(', ')}` : ''} and all ${initialProgress.total} required documents uploaded for ${STUDENT_ID}`);
});
