...
test('Dashboard test', async({page}) => {
    ...
    if (await page.url().includes('dashboard')){
        ...
        await menu.click();
        const secondaryapplications=await page.getByText('Secondary Status Applications',{exact:true});
        await secondaryapplications.click();
        await page.waitForTimeout(7000);
        console.log('Secondary Status page are opened');
    }
});