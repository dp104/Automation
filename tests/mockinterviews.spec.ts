...
test('Dashboard test', async({page}) => {
    ...
    if (await page.url().includes('dashboard')){
        ...
        await newpage.getByRole('button',{name:'Mock-Interviews'}).click();
        ...
        await newpage.url().includes('&companyId=8&branchId=0&tab=Mockinterviews');
        await newpage.waitForTimeout(3000);
        console.log('Mock-Interviews tab are open');
    }
});