...
test('Vivek Consultancy — Add Student (Old): Full Page + Create Application', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/#\/add-student/, { timeout: 10000 });
    console.log('✓ URL: #/add-student');
    ...
    const hasFetchBox = await studentInput.isVisible({ timeout: 5000 }).catch(() => false);
    if (hasFetchBox) {
        ...
        console.log('✓ Student data loaded via fetch');
    } else {
        await page.waitForTimeout(2000);
        console.log('✓ Own profile auto-loaded (no fetch-by-ID box in the new wizard)');
    }
