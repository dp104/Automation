...
test('Vivek Consultancy — Smart Assessment Flow', async ({ page, context }) => {
    ...
    const newPage = context.pages().find(p => p !== page && /add-application/i.test(p.url()));
    // Case B: same-tab redirect
    const sameTab = /add-application/i.test(page.url());
    ...
    if (popupOpen) {
        ...
        if (verdict === 'stuck' || verdict === 'frozen') {
            ...
            // results are all Postgraduate courses for this student.
    ...
    console.log('  themselves render correctly — the apply action is broken for student users.');
