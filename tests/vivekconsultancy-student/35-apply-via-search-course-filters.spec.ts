...
// Flow 3 — Search-Course (Beta) with all filters:
// Set every filter in the search row (Nationality, Destination Country,
// Academic Level, University, Intake), run "Search Programs", then apply
// on a course tile from the filtered results.
// Note: all .msd-trigger buttons need { force: true } (overlay intercepts clicks).

test('Vivek Consultancy — Apply via Search-Course Filters', async ({ page }) => {
