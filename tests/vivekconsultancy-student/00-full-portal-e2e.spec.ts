...
test('Vivek Consultancy — Full Student Portal E2E', async ({ page }) => {
    ...
    const uniCount = await page.locator('[class*="uni-card"], [class*="university-card"], [class*="uni-item"]').count();
    ...
    // 7. UNIVERSITY DETAIL
    // ═══════════════════════════════════════════════════════════════════════
    const firstUniCard = page.locator('[class*="uni-card"], [class*="university-card"], [class*="uni-item"]').first();
    if (await firstUniCard.isVisible({ timeout: 3000 }).catch(() => false)) {
        await firstUniCard.click();
        await page.waitForTimeout(2000);
        const detailVisible = await page.locator('[class*="uni-detail"], [class*="university-detail"]')
            .first().isVisible({ timeout: 3000 }).catch(() => false);
        console.log('✓ 7. University detail opened:', detailVisible);

        // Check tabs on detail page
        const detailTabs = await page.locator('[class*="tab-btn"], [class*="detail-tab"]').count();
        if (detailTabs > 0) {
            await page.locator('[class*="tab-btn"], [class*="detail-tab"]').nth(1).click().catch(() => {});
            await page.waitForTimeout(1000);
    ...
    await page.waitForTimeout(3000);
    const courseCount = await page.locator('[class*="course-card"], [class*="course-item"]').count();
    console.log('✓ 8. Courses page — cards:', courseCount);

    // ═══════════════════════════════════════════════════════════════════════
    // 9. SEARCH COURSE
    // ═══════════════════════════════════════════════════════════════════════
    await page.goto(`${BASE}/#/search-course`);
    await page.waitForTimeout(2000);
    ...
    if (await searchInput.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        await page.waitForTimeout(2000);
        const results = await page.locator('[class*="result"], [class*="course-card"]').count();
        console.log('✓ 9. Search Course — MBA results:', results);
    } else {
        console.log('✓ 9. Search Course page loaded (no search input found)');
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    await page.goto(`${BASE}/#/add-application`);
    await page.waitForTimeout(3000);
    await page.waitForFunction(
    ...
    const accomLoaded = await page.locator('[class*="accommodation"], [class*="accom"]')
        .first().isVisible({ timeout: 5000 }).catch(() => false);
    console.log('✓ 13. Accommodation page loaded:', accomLoaded);

    const accomCards = await page.locator('[class*="accom-card"], [class*="property-card"]').count();
    console.log('    Cards:', accomCards);

    const accomSearch = page.locator('input[placeholder*="search" i], input[placeholder*="location" i]').first();
