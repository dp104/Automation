...
test('Vivek Consultancy — Create Application + Upload Documents', async ({ page }) => {
    ...
    // Upload the dummy PDF into each required-document dropzone.
    // File inputs may re-render after each upload, so re-query every round.
    ...
    for (let round = 0; round < initialProgress.total + 4; round++) {
        ...
        if (!hasLeftover) {
            ...
            if (!inputEl) {
                ...
                if (!anyNeeding && uploads > 0) {
                    console.log('✓ No document cards still need files — uploads complete');
                    break;
                }
                console.log('  no needing document card found — waiting');
                await page.waitForTimeout(3000);
                continue;
            }
            await inputEl.setInputFiles(DUMMY_PDF);
            await inputHandle.dispose();
            console.log('  → staged Dummy_Pdf.pdf in next needing document card');
            await page.waitForTimeout(1500);
        }
    ...
    // Success = counter shows done==total, OR the counter and all "still need this"
    // badges are gone (the UI removes both once every required document is uploaded).
    await ensureTab('Documents');
    const finalProgress = await readDocProgress();
    const stillNeeding = await page.evaluate(() =>
        /still needs? this/i.test(document.body.innerText));
    console.log(`\n  Documents progress after upload: ${finalProgress.done} of ${finalProgress.total} | cards still needing files: ${stillNeeding}`);

    const docsComplete =
        (finalProgress.total > 0 && finalProgress.done >= finalProgress.total)
        || (!stillNeeding && uploads > 0);
    expect(docsComplete, `all ${initialProgress.total} required documents should be uploaded (uploads performed: ${uploads})`)
        .toBe(true);
