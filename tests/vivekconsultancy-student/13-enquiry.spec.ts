...
test('Vivek Consultancy — Enquiry Form', async ({ page }) => {
    ...
    // ── Open sidebar and expand Enquiry ───────────────────────────────────────
    await page.locator('.menu-toggle-icon').click();
    await page.waitForTimeout(800);

    await page.locator('.menu-item').filter({ hasText: /^Enquiry$/ }).first().click();
    await page.waitForTimeout(800);

    // ── Verify sub-menu item ───────────────────────────────────────────────────
    const createEnquiryLink = page.locator('.sub-menu .menu-link').filter({ hasText: 'Create Enquiry' }).first();
    await expect(createEnquiryLink).toBeVisible();
    console.log('Create Enquiry sub-menu item visible ✓');

    await createEnquiryLink.click();
