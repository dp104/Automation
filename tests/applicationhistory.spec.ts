...
test('Dashboard test', async({page}) => {
    ...
    if (await page.url().includes('dashboard')){
        ...
        await page.getByTitle(/View Details/).nth(4).click();
        const newpage = await newtab; //actual value
        console.log("Accodon Details are opened");
        await newpage.getByRole('button',{name:'Application History'}).click();
        ...
        await newpage.url().includes('&companyId=8&branchId=0&tab=History');
        await newpage.waitForTimeout(3000);
        console.log('Application History tab are open');
    }
});